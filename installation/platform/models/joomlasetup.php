...
class AngieModelJoomlaSetup extends AngieModelBaseSetup
{
	...
	public function hasHtpasswd()
	{
		...
		$publicFolder = AngieHelperPublicfolder::getPublicFolder() ?: APATH_ROOT;
	...
	public function hasPhpIni()
	{
		...
		foreach ($files as $file)
		{
			if (file_exists(APATH_ROOT . '/' . $file) || file_exists($publicFolder . '/' . $file))
			{
				return true;
			}
		}
	...
	protected function replaceHtaccess()
	{
		...
		if (!empty($orig))
		{
			if (!file_put_contents($publicFolder . '/htaccess.bak', $orig))
			{
				return false;
			}
		}
		...
		{
			if (!@unlink($publicFolder . '/.htaccess'))
			{
				return false;
			}
		}
	...
	private function removePhpini()
	{
		...
		$renameFiles = [
			APATH_ROOT . '.user.ini',
			APATH_ROOT . 'php.ini',
			APATH_ROOT . 'administrator/.user.ini',
			APATH_ROOT . 'administrator/php.ini',
