...
if (((!empty($images->image_intro)) && $post_format == 'standard') || (is_string($image) && !empty($image))) {
    if ($image_position == 'left' || $image_position == 'right' || $image_position == 'bottom') {
        ...
        if ($image_position == 'left' || $image_position == 'right') {
            echo '<div class="uk-card-media-'.$image_position.($image_position == 'right' ? ' uk-flex-last@l' : '').$image_width_xl.$image_width_l.$image_width_m.$image_width_s.$image_width.'"><div class="uk-cover-container uk-height-1-1">';
        } else {
